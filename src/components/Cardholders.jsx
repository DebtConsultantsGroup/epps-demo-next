...
const formatStatementMoney = (value) => {
...
const normalizeFields = (item, fields) => fields.reduce((acc, field) => {
...
const Row = ({ row, isExpanded, onExpand, onSnackbar }) => {
  ...
  const buildStatementTransactions = (eftItems, feeItems) => {
    ...
    const currentBalance = parseCurrency(row.balance);
  ...
  const loadStatements = async (forceRefresh = false) => {
    ...
    try {
      ...
      setStatementRows(buildStatementTransactions(eftList, feeList));
